...
| 2 | Build the route/component inventory. | `docs/frontend-inventory.md` reconciles the 297-destination source inventory with 18 content archetypes plus one temporary-redirect archetype, React/data/CSS owners, shared patterns, responsive and interactive states, legacy-to-Font-Awesome mapping, and the 27-root-plus-five-banner-dependency migration acceptance slice. | Complete as an ownership inventory; 31 accepted documents render managed content, one accepted owner is an explicit 302, and the 263 plan-only document bodies remain separate content-migration work. |

...
| CA-002 | Fix the original production scope at 51 Payload page documents + 1 redirects-owned source-page route + 2 virtual indexes + 243 listing children = 297 routes                                                                        | Superseded by CA-027                                                       | Retained as the pre-People/Event baseline; it is no longer the current acceptance total                                                                                                                                                                                       |
| CA-003 | Include FAQ (`7609`) and published-banner-reachable EEX News (`11299`); exclude Commodities Report (`2233`)                                                                                                                         | Accepted                                                                   | Public/local navigation drift and banner route provenance are resolved explicitly                                                                                                                                                                                             |
...
| CA-013 | Retain the live HubSpot form identifiers and reproduce active forms through one bounded Next.js embed integration                                                                                                                   | Accepted; local model/inert renderer delivered, provider behavior deferred | Unused WordPress form-builder/admin architecture remains excluded; real submissions wait for an approved test destination and consent flow                                                                                                                                    |
...
| CA-021 | Bridge not-yet-imported Navigation/Footer destinations to canonical `https://www.trayport.com/` URLs                                                                                                                                | Accepted; implemented for the pilot                                        | Accepted managed destinations and the two virtual indexes stay internal; exactly 30 unique live fallback paths are validated (25 leaf destinations plus five clickable section roots), and each is individually reversible when its route passes migration/runtime acceptance |
...
| CA-027 | Retain all 17 public People profiles and consolidate five legacy Event records into the canonical Event Article family, producing 51 Pages + 1 redirects-owned source route + 2 virtual indexes + 263 listing children = 317 routes | Accepted; evidence verified and local route/model implementation delivered | People become first-class Payload route owners; the Event family has 23 canonical `/event/` owners and five `/events/` aliases without duplicate documents                                                                                                                    |

## Architecture milestone completion

The architecture milestone is complete because:

- the public content boundary and 317-route formula are approved;
- FAQ and Commodities decisions are explicit;
- all observed source layout/taxonomy/shortcode semantics have a declared
  disposition;
- 19 content-route archetypes plus the temporary Contact redirect archetype and their route policies are defined;
- collection/global/application-data ownership is assigned;
- implemented and planned block catalogues are separated;
- all 19 content-route runtime archetypes now have enforced route/discriminator
  publication behavior, including publication denial for conversion pages while
  the bounded HubSpot component lacks approved provider/consent behavior and the exactly-one component rule for
  Market Matrix pages;
...
CA-013, CA-014, and CA-022–CA-027 are reflected in the versioned
...
- exact `51 + 1 + 2 + 263 = 317` acceptance;
- deterministic Page reachability from every published banner, including
  `/eex-news/` (`11299`) via banner `11602`;
...
- the implemented foundation already has generated Payload types/migrations,
  schemas and hooks for all 19 content-route archetypes, the People collection, Event consolidation, learning-video/category collections,
...
plan deterministically describes 317 routes: 315 Payload documents (71 marked

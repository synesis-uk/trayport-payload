...
named migration acceptance slice now contains 26 roots: 25 rendered content routes and the explicit
`/request-a-demo/` 302. The other 268 planned bodies remain content-migration work and must not be
...
| `/company/about-us/`; `/company/offices/`; `/company/careers/`; `/resources/faqs/`; `/regions/europe/`; `/regions/asia-pacific/`; `/regions/north-america/`; `/contact/` | `pages`; `page.standard` | `PageView` -> imported hero or fallback page header -> content sections. Offices use structured `office` components; FAQs use disclosures; region pages use managed market-coverage/feature components. Contact deliberately omits two dormant HubSpot form blocks while retaining support details and four offices | Payload page/layout and referenced office, person, reusable-content, region, hub and media records | All named routes are in the 25-route rendered acceptance set. **Implemented behavior / Bridge styling:** Contact is a documented simplification and exact route-level visual acceptance remains open. |
...
Navigation and footer links use an explicit migration bridge. Links to the 26 accepted roots and the
two virtual indexes stay internal. The 35 unique same-site destinations whose bodies are still
plan-only resolve to canonical `https://www.trayport.com/` fallbacks: 30 leaf destinations plus the
...
| --- | --- | --- | --- |
| Global shell | `Header/Component.tsx`, focused desktop/mobile/search clients, thin `Footer/Component.tsx` adapter, typed footer presentation pieces, `CookieNotice.tsx` | Every frontend route | Payload adapters and interactive boundaries are implemented. Footer content/behavior contracts now match the active reference; final route-level pixel acceptance remains **open**. |
...
| Content-section components | heading, rich text, actions, media, feature list, statistics, FAQ, entity list, timeline, data table, gallery, divider, market coverage, embed, data chart and office adapters | Columns inside `contentSection` | Every current Payload union member has a typed adapter and explicit presentation model. **Implemented; route styling acceptance remains separate.** |
...
| Specialist data visualization | `DataChartPresentation`, `MarketVolumeChart.client.tsx`, `MarketCoveragePresentation`, `DynamicConnectionsMap.client.tsx`, `HubView`, and `IndexViews` | Home charts/maps, hub fallback map, coverage index | Deliberate specialist boundary. Highcharts loads through one same-origin runtime. Home retains a typed SVG/managed-media fallback and can viewport-lazy-load pinned Mapbox from application-owned topology; detail/index maps remain server-owned and do not load Mapbox. |
...
| Footer/cookie notice | thin `Footer/Component.tsx` adapter plus `FooterPresentation`, brand, navigation, supporting and legal pieces | cached Footer and Site Settings globals populated by the WP importer | `CookieNotice` versioned aggregate accept/reject record, cross-tab subscription and document consent state | `parity-shell.css` plus older superseded footer/cookie recipes in `globals.css` |
...
| Maps | server `MarketCoveragePresentation`; dark Home-only `DynamicConnectionsMap.client` -> `ConnectionsMapRuntime.client`; `HubView`; `IndexViews` | Payload media, Hub coordinates, Asset Class/Region relationships, bounded line/marker controls; server-only validated provider configuration | Home viewport activation, Mapbox lifecycle/readiness/error fallback and marker labels only when approved configuration exists; the bounded light presentation and all detail/index maps remain server-rendered | map rules in parity/shared CSS; deterministic SVG fallback plus application-derived Mapbox sources/layers in the owning runtime |
...
cookie persistence, article and learning filters, Radix interactive primitives, Highcharts, the
optional viewport-lazy Home Mapbox runtime, video playback, preview/admin helpers, and no broader
route wrapper.

...
import type { Article, Hub, LearningVideo, Page, Person, RouteRegistry, Venue } from '@/payload-types'
import { getServerSideURL } from '@/utilities/getURL'

type ContentOwner = Article | Hub | LearningVideo | Page | Person | Venue
type ContentOwnerCollection = Extract<
  RouteRegistry['ownerCollection'],
  'articles' | 'hubs' | 'learning-videos' | 'pages' | 'people' | 'venues'
>
...
export const getContentSitemap = async () => {
  ...
  const peopleIDs = ownerIDs(claims, 'people')

  const [pages, articles, hubs, venues, learningVideos, people, routeIndexes] = await Promise.all([
    pageIDs.length
      ? payload.find({
          collection: 'pages',
          depth: 0,
    ...
    peopleIDs.length
      ? payload.find({
          ...
          depth: 0,
          draft: false,
          limit: peopleIDs.length,
          overrideAccess: false,
          pagination: false,
          where: {
            and: [{ id: { in: peopleIDs } }, { _status: { equals: 'published' } }],
          },
        })
      : null,
    payload.findGlobal({
